...
local GetSpecialization = _G.GetSpecialization or function() return 0 end
...
function ReforgeLite:ComputeReforge (initFunc, optionFunc, chooseFunc)
  ...
  if success then
    ...
    self.methodDebug = string.format("version = %s\n\n",tocversion)
...
ErrorFrame.text = CreateFrame ("EditBox", "ReforgeLiteErrorFrameText", ErrorFrame.scroll, "BackdropTemplate")
...
function ReforgeLite:DebugMethod ()
  ...
  ErrorFrame:DisplayError ("github.com/Road-block/ReforgeLiteClassic/issues", self.methodDebug or "<no data>")
end
